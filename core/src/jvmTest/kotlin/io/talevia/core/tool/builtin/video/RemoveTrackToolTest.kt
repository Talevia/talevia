...
/**
 * `remove_track` is the complement to `add_track`. The tests exercise the
 * empty-drop happy path, the non-empty guard rail (throws without force,
 * succeeds with force=true), the missing-track guard, the snapshot emission
 * that `revert_timeline` relies on, and the "other tracks stay put" property
 * that makes this safe to use in multi-track projects.
 */
class RemoveTrackToolTest {
    ...
    @Test fun dropsEmptyTrackCleanly() = runTest {
        ...
        val out = rig.tool.execute(RemoveTrackTool.Input("p", "v1"), rig.ctx)
        assertEquals("v1", out.data.trackId)
        assertEquals("video", out.data.trackKind)
        assertEquals(0, out.data.droppedClipCount)
    ...
    @Test fun nonEmptyTrackWithoutForceThrows() = runTest {
        ...
        val ex = assertFailsWith<IllegalStateException> {
            rig.tool.execute(RemoveTrackTool.Input("p", "v1"), rig.ctx)
        }
        val msg = ex.message!!
        // Message mentions clip count and the force=true escape hatch.
        ...
        // Track + clips are untouched after the failed call.
        val refreshed = rig.store.get(rig.projectId)!!
        assertEquals(1, refreshed.timeline.tracks.size)
    ...
    @Test fun nonEmptyTrackWithForceDropsTrackAndClips() = runTest {
        ...
        val out = rig.tool.execute(RemoveTrackTool.Input("p", "v1", force = true), rig.ctx)
        assertEquals(2, out.data.droppedClipCount)
        assertEquals("video", out.data.trackKind)

        val refreshed = rig.store.get(rig.projectId)!!
        assertEquals(1, refreshed.timeline.tracks.size)
        assertTrue(refreshed.timeline.tracks.single() is Track.Audio)
        // Duration is recomputed from surviving clips' tail (audio clip ends at 4s).
    ...
    @Test fun removingOnlyTrackLeavesTimelineEmpty() = runTest {
        ...
        rig.tool.execute(RemoveTrackTool.Input("p", "v1", force = true), rig.ctx)

        val refreshed = rig.store.get(rig.projectId)!!
        assertEquals(0, refreshed.timeline.tracks.size)
        // No surviving clips → duration collapses to zero.
    ...
    @Test fun missingTrackIdThrows() = runTest {
        ...
        val ex = assertFailsWith<IllegalStateException> {
            rig.tool.execute(RemoveTrackTool.Input("p", "ghost"), rig.ctx)
        }
    ...
    @Test fun emitsSingleTimelineSnapshot() = runTest {
        val rig = newRig(
            Project(
                id = ProjectId("p"),
                timeline = Timeline(
        ...
        // Snapshot reflects the post-mutation timeline (v1 gone).
    ...
    @Test fun otherTracksAreUntouched() = runTest {
        ...
        // Audio clip content preserved verbatim.
