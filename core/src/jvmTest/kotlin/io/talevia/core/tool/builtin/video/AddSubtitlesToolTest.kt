...
class AddSubtitlesToolTest {
    ...
    @Test fun dropsAllSegmentsInOneMutationAndOneSnapshot() = runTest {
        ...
        val out = tool.execute(
            AddSubtitlesTool.Input(
                ...
                segments = listOf(
                    AddSubtitlesTool.Segment("hello", 0.0, 1.5),
                    AddSubtitlesTool.Segment("world", 1.5, 1.0),
                    AddSubtitlesTool.Segment("!", 2.5, 0.4),
                ),
            ),
            ctxCollecting(parts),
        )
    ...
    @Test fun sortsOutOfOrderSegmentsByStart() = runTest {
        ...
        tool.execute(
            AddSubtitlesTool.Input(
                projectId = projectId.value,
                segments = listOf(
                    AddSubtitlesTool.Segment("later", 5.0, 1.0),
                    AddSubtitlesTool.Segment("earlier", 1.0, 1.0),
                    AddSubtitlesTool.Segment("middle", 3.0, 1.0),
                ),
            ),
            ctxCollecting(parts),
        )
    ...
    @Test fun createsSubtitleTrackWhenProjectHasNone() = runTest {
        ...
        tool.execute(
            AddSubtitlesTool.Input(
                projectId = projectId.value,
                segments = listOf(AddSubtitlesTool.Segment("hi", 0.0, 1.0)),
            ),
            ctxCollecting(parts),
        )
    ...
    @Test fun extendsTimelineDurationToCoverTailSegment() = runTest {
        ...
        tool.execute(
            AddSubtitlesTool.Input(
                projectId = projectId.value,
                segments = listOf(
                    AddSubtitlesTool.Segment("a", 0.0, 0.5),
                    AddSubtitlesTool.Segment("b", 8.0, 2.5),
                ),
            ),
            ctxCollecting(parts),
        )
    ...
    @Test fun rejectsEmptySegments() = runTest {
        ...
        assertFailsWith<IllegalArgumentException> {
            tool.execute(
                AddSubtitlesTool.Input(projectId = projectId.value, segments = emptyList()),
                ctxCollecting(parts),
            )
        }
    }
