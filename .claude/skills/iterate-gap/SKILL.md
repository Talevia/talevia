---
name: iterate-gap
description: 从 docs/BACKLOG.md 挑最高优先级任务，plan → 实现 → commit body 承载 rationale → 推 main。backlog 空了再按 rubric 一次性补 20 条。参数 "<count> [parallel]"，例：/iterate-gap、/iterate-gap 3、/iterate-gap 4 parallel。执行期间零提问。
---

# iterate-gap — backlog-driven 补齐愿景 gap 的循环

挑当前仓库与北极星之间优先级最高的 gap，plan → 在 `main` 上实现 → commit body 承载 rationale → 推送。**执行期间不向用户提任何问题** —— 每一个决策都按 `docs/VISION.md` 与业界共识自主做出，理由写进本 cycle 的 commit message body，由用户事后异步审阅 / 通过 `git log` 回溯。
...
- **提问**：零次。遇到决策点按 VISION + 业界共识自行决定，理由写进本 cycle 的 commit message body。如果真的卡在只有用户能回答的问题（专有 key、产品抉择、品牌偏好），**换一个 gap**（从 backlog 的下一条取），不要干等，也不要问。
...
- **Rationale 强制随行**：每一轮只做**一次 commit**，message 用 `feat(...)` / `fix(...)` / `refactor(...)` / `docs(...)` 之类按内容贴合的前缀，单句 subject 讲清本轮改了什么；**body 承载完整 rationale**（见 §6 结构）。这一个 commit 同时包含：① 代码改动；② `docs/BACKLOG.md` 里对应 bullet 的删除；③ 可选的 "顺手记 debt" P2 append。`git log -S '<slug>'` / `git log --grep '<slug>'` 天然就是回溯入口。
...
**Milestone 软优先级**（仅在 `docs/MILESTONES.md` 存在且有 `> Current: M<N>` 指针时生效）：**同档内**，bullet 行尾带 `Milestone §M<current>` tag 的靠前，带 `Milestone §later` 或无 tag 的靠后。**不跨档升降** —— 非当前 milestone 的 P0 仍优先于当前 milestone 的 P1，避免 milestone 挤掉合法 debt / correctness fix。tag 不写 = 当成 `§later` 处理。
...
历史数据：4/11 ≈ 36% 的 bullet 在真正 dispatch 时才发现症状已被先前 refactor 默默解决（早期 skip-close 的 commit 在 `git log --grep 'skip-close'` 能看到）。为避免每次都交"走一遍代码 → 解释 no-op"的 tax，**挑中 bullet 后先花 ≤ 60 秒**尝试复现它声称的症状：
...
- 删 BACKLOG 对应 bullet。
- Commit message subject `docs(backlog): skip-close <slug> — already resolved by <shorthash>`；body 一段说明原症状 + 命中的 refactor commit（shorthash + 一句摘要）。
...
3. `git log --oneline -60` + `git log --grep '^feat\|^refactor\|^fix' -30 --format='%h %s%n%n%b'` —— 最近落地的内容 + commit body 里的 rationale。
...
5. **Perf 工作停更** —
   ```
   git log --oneline --since='3 months ago' --grep='perf\|latency\|cost\|token\|budget\|benchmark\|incremental'
   ```
   若最近 3 个月零相关 commit → 提示本轮 repopulate 至少考虑一条 perf gap（**非硬性**，但连续零 perf 工作本身就是隐性劣化信号）。
...
- 每条 bullet 格式：`- **<slug>** — <Gap：现状 / 痛点>。**方向：** <期望动的东西>。Rubric §5.x。Milestone §M<N> | §later。`
- **Milestone tag** —— 读 `docs/MILESTONES.md` 的 `> Current: M<N>` 指针，判断每条 bullet 是否**直接推进**当前 milestone 的某条未勾 exit criterion：是 → `Milestone §M<N>`；否（包括价值认可但属于后续 M 的）→ `Milestone §later`。不确定归 `§later`（软优先级下不伤害；`§M<N>` 错标会让不相关的工作在 milestone gate 下抢位）。`MILESTONES.md` 缺失则一律 `Milestone §later`。
- `<slug>` 用 kebab-case，一眼看出改什么（例 `unify-project-query`、`split-project-json-blob`）。
...
7. **序列化向前兼容** — 新加 `@Serializable` 字段必须有 default 值（否则旧 JSON / SQLite blob 解不出来，迁移成本高得离谱）。删字段前 `git log -S '<fieldName>' --all -- docs/ CLAUDE.md` 看文档 / commit body 有没有依赖。`kotlin.time.Duration` 不加自定义 serializer（历史名字碰撞事故）。
...
### 6. 删 backlog bullet + commit body 承载 rationale

本 cycle 不写独立决策文件。本次改动的 rationale（context / decision / alternatives / coverage / registration）直接进**本轮 commit 的 message body**——`git log --format='%H %s%n%n%b'` 可聚合、`git log -S '<slug>'` 可回溯，承载力与过去的 `docs/decisions/*.md` 一致但没有文件维护成本。

**Commit body 的推荐结构**（非强制格式，但"信息齐"是硬要求）：

```
<subject: 一句话讲本轮核心改动>

Context. 这个 gap 为何本轮必做（rubric 轴 + 当前代码里观察到了什么 +
本轮 rubric delta 例："§5.1 source-layer 序列化 无 → 部分"）。

Decision. 落地了什么 —— 关键类型名、tool 名、文件。

Axis. 仅 split / extract / dedup / refactor 时必填，一句话回答"什么
东西增长会重新把这个文件 / 类吹回阈值"；非结构改动填 n/a。

Alternatives. 至少两个被拒的候选，每个配一句拒绝理由。业界共识要
具体到名字（kotlinx.serialization / OpenCode tool-dispatch / SemVer）才算数。

Coverage. 哪些测试覆盖了本次改动。

Registration. 动了哪几个装配点（或"无需注册 —— 纯 schema / 纯内部重构"）。
```

Commit message **subject 一句话**足以让 `git log --oneline` 扫到；body 长度不限（git 不收费）。这些字段的完整度是 reviewer / 后续 cycle 的 agent 判断本次改动是否站得住脚的唯一依据，**不要省略**。

**删 backlog bullet**：把本轮处理的 bullet 从 `docs/BACKLOG.md` 删掉：
...
```
- **debt-<slug>** — <发现了什么>。**方向：** <建议的修法>。Rubric 外 / 顺手记录。Milestone §later。
```

规则：
- 只能 append 到 P2 末尾，不能插队到 P0 / P1，不能重排既有 bullet。
- 一次 cycle 最多 append 2 条（发现更多说明跑偏了，用报告提醒用户）。
- 和本轮的代码改动 + bullet 删除一起进**同一次 commit**（见 §7）。
...
- **一次 commit**，同时包含：
  ...
  ⑥ **可选** `docs/MILESTONES.md` tick / promote 产物（由 §M 决定，必须和 ①–⑤ 同 commit 才进 main，见 §M.5）。
- Commit message 前缀按 `git log --oneline -20` 的惯例选最贴合内容的一个（当前：`feat(core):`、`fix(...)`、`refactor(...)`、`docs(...)`）。Subject 一句话讲本轮核心改动；**body 按 §6 的结构承载完整 rationale**（Context / Decision / Axis / Alternatives / Coverage / Registration）——这是本轮改动唯一的 rationale 入口，未来 `git log -S '<slug>'` 和 `git log --grep '<slug>'` 都靠它。
...
Backlog repopulate 那次的 commit 是独立的 `docs(backlog): …`（见 R 小节），不跟任何 feat commit 绑定 —— 它只改 `docs/BACKLOG.md` 一个文件，rationale 直接进 commit body（scan 结果 + 本轮 rubric 打分摘要）。

### §M. Milestone sync（§7 push 成功之后跑，§8 之前）

**先决条件**：`docs/MILESTONES.md` 存在且有 `> Current: M<N> —` 指针。缺失则整节跳过。**§7 push 失败（rebase 冲突等）不跑本节**——evidence 必须在远端 main 上才算数。

**1. Auto-tick evidence-based criteria**：对当前 milestone block 里每条 `[ ]`，若其文本包含 grep-able 子句（形如 `grep: <pattern>` 或具名类 / 函数 / 文件 / 常量），跑对应检查：

- `grep: <pattern>` → `git grep -nE '<pattern>'` 非空 **且至少一条命中在产品路径**（`core/src/*Main/kotlin/`、`apps/*/src/main/kotlin/`、`platform-impls/*/src/main/kotlin/`，排除 `*Test/` / `build/`）。
- `test 覆盖` / `e2e test` → `grep -rnE '@Test.*<name>|fun\s+<name>\s*\(' core/src/*Test apps/*/src/test --include='*.kt'` 找到对应测试。
- 可选佐证：`git log --since='14 days ago' -S '<pattern>'` 非空（落地时间窗过近，排除 grep 命中的是老残留的情况）。

全部命中 → 把 `[ ]` 改成 `[x]` 并在行末 append ` — cycle <date> <shorthash>`（shorthash = 本 cycle 的 feat commit）。**部分命中不勾**（比如 grep 到了但 test 没加 → 下次来）。

**2. 纯主观 criterion 不自动勾**：标记有 "必须手动 tick" 字样的（例：milestone 退出总结）。只有当本 cycle 已在 `docs/MILESTONES.md` 对应位置 append 了明确的退出总结段落（剩余 gap + commit shorthash 列表）时才能勾；段落和 tick 必须同 commit 落盘。

**3. Auto-promote（全勾 → 触发）**：若当前 M 的所有 criteria 全是 `[x]`：

- 顶部 `> Current: M<N>` → `> Current: M<N+1>`。
- 从 "亚军 milestones" 列表里取最靠前的 candidate 升格为正式 M<N+1> block，按其对应 §3 赌注起草 5–8 条初稿 criteria（大多可 grep 的实体，掺 1 条"milestone 退出决策" 手动勾位）。
- 在 `docs/MILESTONES.md` 底部 append 一个 `## Completed — M<N> (<date>)` block，内含：M<N> 每条 criterion 对应的 commit shorthash（即 `[x] ... — cycle <date> <hash>` 里那些）+ M<N+1> 目标一句话 + criteria 起草逻辑一句话。
- 上述三项 **和触发 promotion 的本 cycle feat commit 同 commit**（不单独 push），保证 "每次 milestone 前进都有一个明确主因" 可查。

**4. 无变化**：本 cycle 没勾到任何 criterion → `§M` 无输出。

**5. Amend 约束**：§M 的 `MILESTONES.md` / 新 decision 文件 append 要进**本 cycle 已推送的 commit 之前**完成——具体做法是 `§7. Commit + push` 里把 §M 的产物一起 stage，再一次 push。§M 不能作为独立补丁 commit（违反硬规则 3 "commit → push 永远配对"）。若 §7 已经 push 出去才发现该勾却没勾，**不追补**——等下次 cycle 自然 tick。
...
- 当前 milestone（从 `docs/MILESTONES.md` 读）+ 本 cycle 是否勾到 criterion / 是否触发 promotion。
...
- 互不重叠的数量 < N → **静默缩减 N** 到最大不重叠子集，最后报告提到缩减。（`docs/BACKLOG.md` 的 bullet 删除天然不冲突 —— 每个 sub-agent 删自己那条 bullet，不同行间互不影响。）
...
2. 复制顺序模式第 2.5–7 步（liveness pre-check → plan → 实现 → 验证 → 归档决策 + 删 bullet → commit）。§2.5 对分配给自己的 bullet 仍然适用 —— 症状无法复现就走 skip-close 路径（纯 decision + bullet delete 的 worktree commit，主调度器合进来时也不冲突）。仅一处调整：
   ...
   - Rationale 照常进 worktree commit 的 message body（按 §6 结构），**不写独立决策文件**。
...
4. 输出契约：最终消息必须包含 commit SHA、被删除的 bullet slug、一句话结果摘要、测试是否绿。失败时不要 commit，把错误返回。
...
3. `git rebase main <branch>` —— 有代码冲突在这里解决。各 sub-agent 的 BACKLOG bullet 删除通常落在不同行，预期不冲突。
...
5. 每一轮 feat / refactor / fix commit 必须带 `docs/BACKLOG.md` bullet 删除 + commit message body 完整 rationale（按 §6 结构）。BACKLOG 默认"只删不改"，唯二合法 in-place 编辑是 §3 skip-tag append 与 §6 "顺手记 debt" P2 append。`docs/PAIN_POINTS.md` / `docs/ENGINEERING_NOTES.md` / `docs/MILESTONES.md` 同样 append-only —— 只新增 cycle-dated / milestone-dated section，不得编辑 / 重排历史条目（MILESTONES.md 允许对当前 milestone 的 exit criteria 做 `[ ] → [x]` 勾选 + 行末 ` — cycle <date> <hash>` 标注，其余内容仍然不改）。Repopulate 的 `docs(backlog)` commit 是例外（它重建 BACKLOG，不配对 bullet 删除）；§2.5 skip-close 也是例外（只删 bullet，没有代码改动；rationale 全在 commit body）。
