...
**2. 纯主观 criterion 不自动勾**：标记有 "必须手动 tick" 字样的（例：milestone 退出决策）。只有当本 cycle 已新建对应决策文件（`docs/decisions/<date>-milestone-<slug>-*.md`）时才能勾，且必须在本 cycle 的 commit 里一起落盘。
...
- 新建 `docs/decisions/<date>-milestone-advance-m<N+1>.md` 决策：列 M<N> 每条 criterion 对应的 commit shorthash + M<N+1> 目标 + criteria 起草逻辑。
