...
class AppContainer(env: Map<String, String> = System.getenv()) {
    ...
    /**
     * Bundle-local blob writer for AIGC tools — persists generated bytes under
     * the project bundle's `media/` directory so `git push` ships them along
     ...
     * that still receive it as a constructor arg (AIGC + import + ML lanes).
     * Their migration to `Project.assets` via `ProjectStore.mutate` happens in
     * a separate task; once those tools no longer need a [MediaStorage] this
     * field — and every use of `media` below — can be deleted.
     */
    ...
    /**
     * TODO(file-bundle-migration): legacy machine-wide [MediaBlobWriter] used by
     * AIGC tools that haven't migrated to [bundleBlobWriter] yet. Lands under
     * `<java.io.tmpdir>/talevia-generated` so AIGC still has somewhere to
     * write; once every consumer uses [bundleBlobWriter] this field can be
     * deleted.
     */
    val blobWriter: MediaBlobWriter = FileBlobWriter(File(System.getProperty("java.io.tmpdir"), "talevia-generated"))
    ...
    val tools: ToolRegistry = ToolRegistry().apply {
        ...
        // TODO(file-bundle-migration): ImportMediaTool + FfmpegProxyGenerator both still
        // accept a MediaStorage / MediaPathResolver. Once they migrate to Project.assets +
        // BundleMediaPathResolver, drop the `media` reference here.
        ...
        // All AIGC tools now persist into the project bundle via bundleBlobWriter
        // (file-bundle migration). `media` is still threaded into the
        // image / video / upscale tools for character_ref reference-image
        // resolution; that resolves to a TODO once those tools migrate to
        // BundleMediaPathResolver.
        imageGen?.let { register(GenerateImageTool(it, media, bundleBlobWriter, projects)) }
        videoGen?.let { register(GenerateVideoTool(it, media, bundleBlobWriter, projects)) }
        musicGen?.let { register(GenerateMusicTool(it, bundleBlobWriter, projects)) }
        upscale?.let { register(UpscaleAssetTool(it, media, bundleBlobWriter, projects)) }
