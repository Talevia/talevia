 ...
 *    purpose.
 *
 * Output lives in a dedicated sibling directory under the caller-provided
 * [proxyDir] (`<proxyDir>/<assetId>/`) so the UI can cheap-list them and the
 * proxy files don't pollute the main storage catalog. When the generator is
 * given no writable parent, it falls back to the system temp dir — still
 * usable, but the files disappear on reboot.
 *
