...
- **Android transition rendering** — `Media3VideoEngine` still no-ops transitions: `add_transition` writes an Effect-track clip to the timeline, but the exported mp4 has hard cuts. FFmpeg and AVFoundation both render transitions as a dip-to-black fade (every `transitionName` collapses to a per-clip alpha ramp at the boundary); Media3 needs the same shape via a custom `GlEffect` (there's no built-in alpha/opacity primitive). LUT grading is real on all three engines via the shared `.cube` v1.0 parser in `core.platform.lut` (`SingleColorLut.createFromCube` on Android; `CIColorCube` on iOS; `lut3d` on FFmpeg). Subtitles are baked on all three engines: FFmpeg via `drawtext`, Media3 via `TextOverlay` + `OverlayEffect`, AVFoundation via `CATextLayer` + `AVVideoCompositionCoreAnimationTool`. Filter parity: Android skips `vignette` (no built-in Media3 primitive; needs a custom `GlShaderProgram`); FFmpeg and iOS render every documented filter.
