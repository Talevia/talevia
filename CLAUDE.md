...
- **Native filter / transition rendering** — `Media3VideoEngine` (Android) bakes `brightness` / `saturation` / `blur` via `Brightness` / `HslAdjustment` / `GaussianBlur` effects; `vignette` and `lut` still no-op there (no built-in vignette; `SingleColorLut` needs a `.cube` parser). `AVFoundationVideoEngine` (iOS) bakes `brightness` / `saturation` / `blur` / `vignette` via `CIFilter` chains keyed on composition time (iOS has a built-in `CIVignette`); `lut` still no-ops pending the same `.cube` loader. Transitions still no-op on both native engines. FFmpeg JVM has full filter coverage including `lut3d`.
