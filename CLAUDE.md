...
- **Native transition rendering** — both `Media3VideoEngine` (Android) and `AVFoundationVideoEngine` (iOS) now render every filter the FFmpeg engine does except `vignette` on Android (no built-in Media3 primitive; needs a custom `GlShaderProgram`). LUT grading is real on all three engines via the shared `.cube` v1.0 parser in `core.platform.lut` (`SingleColorLut.createFromCube` on Android; `CIColorCube` on iOS; `lut3d` on FFmpeg). Subtitles now render on Media3 via `TextOverlay` + `OverlayEffect`; iOS `AVFoundationVideoEngine` still ignores `Track.Subtitle` at render time (follow-up: `CATextLayer` through `AVVideoComposition.animationTool`). Transitions still no-op on both native engines — `add_transition` writes to the timeline but the exported mp4 has hard cuts. Wiring Media3 opacity ramps / AVFoundation `setOpacityRamp` is the follow-up.
