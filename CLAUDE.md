...
- **`UpscaleEngine` has no bundled provider.** The `upscale_asset` tool ships but stays unregistered until `AppContainer.upscale` / `ServerContainer.upscale` is wired. Real-ESRGAN / SUPIR live on Replicate or run locally via ONNX — a Replicate-backed engine mirroring `ReplicateMusicGenEngine` is the obvious first add.
