...
- **`MusicGenEngine` has no bundled provider.** The `generate_music` tool ships but stays unregistered unless a concrete engine is plugged into `AppContainer.musicGen` / `ServerContainer.musicGen`. No mainstream public API exposes MusicGen / Suno / Udio today; Replicate hosts MusicGen and is the obvious first concrete engine to add.
- **`UpscaleEngine` has no bundled provider.** Same story — the `upscale_asset` tool ships but stays unregistered until `AppContainer.upscale` / `ServerContainer.upscale` is wired. Real-ESRGAN / SUPIR live on Replicate or run locally via ONNX; both are environment-specific.
- **Per-clip incremental render is deferred.** `ExportTool` memoizes whole-timeline exports only; there is no "render one stale clip + reuse the rest" path. Stale-guard + `regenerate_stale_clips` keep today's flow correct (exports refuse stale renders, regenerate busts the timeline hash, re-export is fresh), just not efficient on long projects with a small edit. Revisit plan is captured in `docs/DECISIONS.md` under "Per-clip incremental render".
