...
- **DB persistence (sessions only)**: SQLite via `core.db.TaleviaDbFactory` still hosts Sessions / Messages / Parts. Honours `TALEVIA_DB_PATH` (absolute path, `":memory:"` / `"memory"` to force ephemeral). Unset → `~/.talevia/talevia.db` on Desktop / CLI; in-memory on Server unless explicit. Schema versioning uses `PRAGMA user_version`; downgrades refused. (The 4 project-related tables in this DB — `Projects`, `ProjectSnapshots`, `ProjectLockfileEntries`, `ProjectClipRenderCache` — are dead-code as of `baad43f`; deletion is queued in `docs/BACKLOG.md::delete-sqldelight-project-store`.)
