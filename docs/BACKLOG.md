...
- **source-query-orphan-nodes** — `SourceNode` DAG 上可能有零 clip 绑定的节点（用户建模后没实际用上 / clip 换绑定后遗留）。现在只能肉眼扫 `select=nodes` + 每行跟 `select=clips_for_source` 交叉比对 O(N²)。`ascii_tree` 已经内部算过 orphanSet 但没单独暴露。**方向：** `source_query(select=orphans)` 直接返零 clip-bound 的节点列表（id / kind / createdAt），供 cleanup workflow 一次性读出。Rubric §5.1 / §5.5。Milestone §later.
- **benchmark-compaction-wall-time** — 已有 5 个 wall-time bench（agent loop / lockfile lookup / lockfile entries / file-project openAt / export），但 `Compactor.compactIfNeeded` 没有回归守护。200-message session 触发两级算法时 wall-time 无基线，默默变慢没人发现。**方向：** jvmTest `CompactionBenchmark` —— 200-message history + 若干 tool outputs → 单次 `compactIfNeeded` wall-time 软预算；和现有 bench 一样走 `AgentLoopBenchmark.report` 打印 `[bench]` 行。Rubric §5.7。Milestone §later.
- **provider-query-warmup-stats** — `BusEvent.ProviderWarmup(Starting|Ready)` 已发射（6c460a10 + 7c733de5）但无聚合——用户看不到 "OpenAI image P50 冷启 2s、Sora 15s" 这种关键 latency 画像。**方向：** `provider_query(select=warmup_stats)` 按 providerId 聚合 wall-time 分位数（P50 / P99，last N samples），数据源为 Starting→Ready 间距；后台 subscriber 累积到 `core.metrics` 读写。Rubric §5.7。Milestone §later.
