...
- **delete-sqldelight-project-store** — `baad43f` 把 ProjectStore 切到 `FileProjectStore`，但 `SqlDelightProjectStore` 类 + 4 张 `.sq` 表 (`Projects` / `ProjectSnapshots` / `ProjectLockfileEntries` / `ProjectClipRenderCache`) 仍躺在 `commonMain`，没人读写。容器构造时仍 `SqlDelightProjectStore(db, ...)` 作 stub —— 死代码 + 死表 schema 双重技术债。**方向：** 删 `SqlDelightProjectStore.kt` + 4 个 `.sq` 文件 + 容器里的 stub 构造；bump `TaleviaDb.Schema.version` 3 → 4 加 `migrations/4.sqm` `DROP TABLE` 这 4 张；sessions/messages/parts schema 不动。Rubric §3a-3 / clean-cut。
