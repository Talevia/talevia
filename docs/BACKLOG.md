...
- **debt-split-validate-project-tool** — `ValidateProjectTool.kt` 396 LOC 是 core 当前最长文件（继 ClipMutateHandlers 之后）。所有 validation 检查 (track invariants / clip ranges / lockfile / source DAG / asset references) 在一个 class 里。**方向：** 抽 `ProjectValidationChecks.kt`（per-axis check 函数）+ 主 dispatcher 走 `listOfChecks().map { it.run(project) }`；schema 也分到 `ValidateProjectToolSchema.kt`（按已落地的 *Schema extract pattern）。Rubric §5.6。Milestone §later.
- **provider-query-cost-history** — `MetricsRegistry` 的 `aigc.cost.*` counters 只有累积总数，缺 per-call ledger。`session_query(select=spend_summary)` 给 per-provider rollup，`project_query(select=lockfile_entries)` 给 per-asset 详情；缺 "最近 N 次 AIGC 调用，按时间倒序，每行 (toolId, modelId, cents, sessionId, partId)" 的 streaming view。**方向：** `provider_query(select=cost_history, limit?, sinceEpochMs?)` 走 lockfile entries 跨所有 project 聚合 + sort by `provenance.createdAtEpochMs desc`。Rubric §5.7 / §5.4。Milestone §later.
