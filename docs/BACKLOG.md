...
- **import-session-tool** — `export_session` 在 cycle 113 (`talevia-session-export-v1`) + cycle 115 (`md` rendering) 已就位，但反向 `import_session` 没有。`SessionEnvelope` JSON 没法被另一台机器消费 —— 跨实例 transcript 共享只能"看"不能"接着 agent"。**方向：** 新工具 `import_session(envelope: String, projectId: String)`：解 envelope → `SessionStore.createSession + appendMessage + upsertPart`；版本不匹配 fail loud；目标 project 不存在 fail loud；返回 newSessionId。Rubric §5.4。Milestone §later.
