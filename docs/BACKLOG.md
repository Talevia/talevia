...
- **debt-clip-render-cache-map-lookup** — `ClipRenderCache.findByFingerprint` 是对 `entries: List<ClipRenderCacheEntry>` 的 O(N) 线性扫描。N 在今天的项目下最多几百，找不到热路径证据；但随着 per-clip incremental render 的普及，深缓存（≥ 千条）会让 lookup 成为非 trivial 成本（benchmark 时已经能看到 1–2 ms/op 的 lookup 开销吞没 render 节省 —— 见 `2026-04-23-debt-perclip-benchmark-inverted-scaling-under-stub-engine.md`）。**方向：** 把 `entries` 从 `List<ClipRenderCacheEntry>` 换成 `Map<String, ClipRenderCacheEntry>`（fingerprint 作 key；"later entry wins" 语义通过 put-覆盖天然保留），`findByFingerprint` 变 O(1)。**触发条件：** 实测 profiling 数据支持深缓存场景 `findByFingerprint` 占 export wall time ≥ 5% 的报告。Rubric 外 / 顺手记录。
