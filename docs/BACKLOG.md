...
- **cost-aware-provider-fallback-ordering** — M2 exit summary §3.1 follow-up #3：`AgentProviderFallbackTracker` 只 *记录* 已发生的 hop，Agent 的 fallback chain 本身按 `fallbackProviders` 构造顺序（= registry 装配顺序）迭代，不参考 `AigcPricing` 已知成本。criterion 4 "成本可见" 已给 read 口（`session_query(select=spend_summary)`），但写 / 决策口没有 —— 用户在 budget 紧张时仍会被扔到最贵的 provider 先试。**方向：** 引入 `ProviderRoutingPolicy`（sealed interface，impl: `RegistryOrderPolicy`（现行默认）/ `CheapestFirstPolicy`），`Agent` 在构造 fallback 迭代顺序时咨询 policy + `AigcPricing` 的 model→cost 映射，`Session.metadata` 持 policy choice（default `RegistryOrder`，兼容现行语义）。Rubric §5.2 / §5.7。Milestone §later.
- **provider-warmup-bus-event** — M2 exit summary §3.1 follow-up #4：AIGC 首次调用 provider 的 connection setup / model load / seed-pinning handshake 对用户不可见，session 冷启动首张图 "卡死感" 是目前最直观的 UX 劣化。`BusEvent` 家族没有 warmup 轴。**方向：** 加 `BusEvent.ProviderWarmup(sessionId, providerId, phase=Starting/Ready, epochMs)` + `ReplicateImageGenEngine` / `ReplicateMusicGenEngine` / `ReplicateUpscaleEngine` 在 prediction POST 前 publish `Starting`、首次轮询 200 后 publish `Ready`，CLI / Desktop 的 EventRouter 把事件转成一行 "warming up <provider>…" 提示。Rubric §5.4 / §5.7。Milestone §later.
