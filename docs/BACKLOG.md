...
- **debt-split-source-node-action-tool** — `SourceNodeActionTool.kt` 585 LOC, 超 R.5.4 默认 P1 长文件阈值。`ClipActionTool` 轴-split 先例证明了"create-verb vs mutate-verb"拆法可按文件组织复用。**方向：** 镜像 `ClipActionTool` 的轴 split 模式 —— 识别 add/remove/fork 等创建新 id 的 verbs 为一类，body/parents/rename 改既有节点的为另一类，分到 `SourceNodeCreateHandlers.kt` / `SourceNodeMutateHandlers.kt`，主类保留 dispatcher + schema + rejectForeign。零 LLM-visible 变化。Rubric §5.6。Milestone §later.
- **debt-split-clip-set-action-tool** — `ClipSetActionTool.kt` 502 LOC, 刚到 R.5.4 阈值且还有几个 `field=` 分支留着加（`fade` / 新 blend 模式等）。**方向：** 按 `field` 轴（transform / volume / source_binding）拆成 per-field handler 文件，dispatcher 留在主类。Rubric §5.6。Milestone §later.
