...
- **m2-pin-hit-rate-select** — M2 criterion 3："pin 命中率可见"。`project_query` 没有 select 暴露"export 几次、几条 clip 命中 lockfile cache 没重跑 AIGC"。**方向：** `project_query(select=lockfile_cache_stats)` 聚合 `ClipRenderCache` / `project.lockfile.entries` 的命中 vs miss 数据；schema 含 `{totalExports, cacheHits, cacheMisses, hitRatio, perModelBreakdown}`。Rubric §5.7 / §5.3。Milestone §M2.
...
- **debt-tool-spec-budget-remeasure** — 上轮 22_518/22_600 margin 数字来自 iteration 7 repopulate；iterations 8-12 净减 10 tools，实际 margin 应宽松。若 M2 3 个新 select 落地会加 ~600 tokens。**方向：** 跑 `ToolSpecBudgetGateTest` 取当前值；若 > 22_000 保留预警，若 < 20_000 降 P2 放手做 M2 selects。Rubric §5.7。Milestone §later.
