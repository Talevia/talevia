...
- **debt-centralise-pretty-json-desktop** — `apps/desktop/` 里 `LockfilePanel.kt` / `TimelinePanel.kt` / `SourceNodeHelpers.kt` 都有一个 `val PrettyJson = Json { prettyPrint = true; prettyPrintIndent = "  " }`（`SourceNodeHelpers` 用了 `SourcePrettyJson` 避免名字碰撞）；两个基于 `JsonConfig.default`、一个从零建，语义不完全一致。**方向：** 合并为一个 `DesktopPrettyJson` 文件，统一到 `JsonConfig.default` 基底 + prettyPrint 覆写；三处 callers 切到它。先 decision 评估一下把基底差异默默归一带来的风险（Kotlin `Json` 的 `classDiscriminator` / `ignoreUnknownKeys` 应当对纯 JsonObject 打印无影响）。Rubric 外 / 顺手记录。
